...
timer_state = {
    ...
    'warning_fired': False,
    'expired': False,
    'preset_name': config['timer'].get('active_preset', ''),
}
timer_thread = None
...
def api_timer_start():
    if not timer_state['running']:
        ...
        broadcast('timer_state', timer_state)
        if not was_paused:
            sfx_name = tcfg.get('start_sound', '')
            if sfx_name:
                audio_path = _resolve_audio_file(sfx_name, prefer_music=True)
                if audio_path:
                    start_pos = float(tcfg.get('start_sound_start', 0))
                    fade_sec  = float(tcfg.get('start_sound_fade', 0))
                    def _timer_sound(sp=start_pos, fade=fade_sec, ap=str(audio_path)):
                        play_audio(ap, start_pos=sp)
                        if fade > 0:
                            time.sleep(max(0.0, fade))
                            fade_audio(fade)
...
def api_countdown_start():
    if not countdown_state['running']:
        ...
        if not was_paused:
            ...
            if sfx_name:
                audio_path = _resolve_audio_file(sfx_name, prefer_music=False)
                if audio_path:
                    play_sfx(str(audio_path))
                else:
        ...
        try:
            if action == 'scene':
                ...
                if preset:
                    ...
                    save_config(_cfg)
                    config = _cfg
                    dur = int(preset.get('duration', 180))
                    timer_state['duration'] = dur
                    if not timer_state['running']:
                        timer_state['seconds_remaining'] = dur
                    timer_state['warning_fired'] = False
                    ...
                    broadcast('timer_state', timer_state)
                    broadcast('timer_preset_loaded', {'name': preset_name, 'duration': dur})
...
def api_save_timer():
    ...
    save_config(cfg)
    # Update live timer state duration
    timer_state['duration'] = t['duration']
    return jsonify({'ok': True})
