...
Asked for more: two further real levers, again from reading the actual firmware/library rather than guessing. `setCpuFrequencyMhz(80)` — was running at the ESP32's default 240MHz the entire time despite spending nearly all of it idle (waiting on a poll timer or button press); 80MHz is the floor that still keeps WiFi usable. `M5.Axp.ScreenBreath(60)` — the backlight had never been explicitly set to a brightness at all; it was sitting at the AXP192's power-on register default (confirmed by reading `AXP192.cpp` — register `0x28` resets to a value working out to ~70% on `ScreenBreath`'s own 0-100 scale). Backlight is this device's single biggest draw (per the pre-existing `SCREEN_SLEEP_MS` comment) — the 12s auto-sleep already cuts its *duration*, this cuts its *intensity* the whole time it's actually awake, a second independent lever on the same biggest draw. Both compile-verified clean the same way.
...
### Walk-Up Still Boot Prewarm
Asked directly: Console screens take a long time to load, can they preload at boot? Investigated rather than assuming everywhere was slow — the Display/Memes thumbnail prewarm already existed (earlier session), but circle/role walk-up stills (`_generate_walkup_still`, an ffmpeg last-frame extract) had no equivalent — every first tab-open or first live fire of a still paid that cost live. Added `_prewarm_walkup_stills()`, chained into the same background thread as the existing display-thumb prewarm (not a second parallel thread — ffmpeg work on this Pi already confirmed to just serialize under concurrency, so two competing prewarm threads would only fight each other, not finish faster). `_generate_walkup_still()` already no-ops for anything current, so this is free on every boot after the first.
...
### Show Flow Step Vanished — Load Template Was a Silent Full Replace
Reported: a show flow step added the night before ("Story Tellers") was confirmed working live on Console, then later found missing on Admin. Traced it in the actual live config, not assumed — the step really was gone from the server's `show_flow`, not just a stale Admin display. Root cause: Admin's "Load Template" feature does `cfg['show_flow'] = tmpl['steps']`, a full unconditional replace with zero trace of what was overwritten. Loading a template saved before the step was added would wipe it out completely. Couldn't pin the exact moment — the retained config backups (last 20 saves) didn't go back far enough — but the mechanism fit the symptom exactly. Re-added the step by hand from what Console had shown, verified back in its original position between Buccaneers Skit and Mighty Creek Skit.

Fixed the actual mechanism: before a template load overwrites the live show flow, the current live flow now auto-saves into the same templates list under a clearly-tagged, timestamped entry (sorts to the top, visible AUTO badge). A wrong template load is now a one-click undo instead of unrecoverable data loss, capped to the last 10 auto-backups. Verified live end-to-end: saved the real 24-step flow as a throwaway template, loaded a different real template to trigger the new path, confirmed the auto-backup appeared correctly (with Story Tellers intact inside it), then restored the real state and confirmed nothing was left altered.

### Normalize Now Covers a Walkup Video's Own Audio
Asked directly, following the walkup-video-audio question above: normalize music/SFX/walkups consistently, since there's a real loudness mismatch walkup to walkup. Root cause: Normalize already covered every other audio source (music, SFX, walkup_music files, karaoke) but never touched a walkup video's own embedded audio track — the newly-added MUTE VIDEO checkbox path — so those played at whatever level the source video happened to have, never leveled like everything else. `_collect_norm_files` now also picks up circle/role/game_entry video files that actually carry an audio stream (probed first, so silent/muted walkup videos — most of them — are never queued for nothing); `_normalize_file_2pass` keeps the video stream byte-identical (stream-copy) and only touches the audio, using the same two-pass EBU R128 technique already proven for every other file type.

Ran it for real against all 179 audio/video files (music, SFX, walkups, karaoke) and found two genuine bugs along the way, both fixed live: a stale leftover temp file (the library's own in-progress-file naming pattern) got mistaken for a real source and failed harmlessly but noisily — now guarded against for good; and a real crash where ffmpeg echoed a non-UTF-8 metadata byte from one SFX file's tags back through its stderr, tripping Python's default UTF-8 decode — fixed with `errors='replace'` on the diagnostic text capture (never touches the actual audio). Both affected files were manually finished afterward once fixed. Verified live: video streams post-normalize are still byte-identical (resolution/codec unchanged, duration within milliseconds), and re-measured loudness on a walkup video with real audio landed right at the -14 LUFS target instead of wherever it started.
